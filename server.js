...
function localExpertEngine(input) {
  ...
  if (fastMa > slowMa) {
    bullish += 20;
    reasons.push("Fast moving average is above slow moving average, supporting bullish trend continuation.");
  }

  if (fastMa < slowMa) {
    bearish += 20;
    reasons.push("Fast moving average is below slow moving average, supporting bearish trend continuation.");
  }

  if (rsi >= 55 && rsi <= 68) {
    bullish += 14;
    reasons.push("RSI is bullish but not extremely overbought.");
  }

  if (rsi <= 45 && rsi >= 32) {
    bearish += 14;
    reasons.push("RSI is bearish but not extremely oversold.");
  }

  if (rsi > 72) {
    bearish += 12;
    warnings.push("RSI is very high; buying may be late and reversal risk is elevated.");
  }

  if (rsi < 28) {
    bullish += 12;
    warnings.push("RSI is very low; selling may be late and bounce risk is elevated.");
  }

  if (price > resistance) {
    bullish += 18;
    reasons.push("Price is trading above resistance, suggesting breakout pressure.");
  }

  if (price < support) {
    bearish += 18;
    reasons.push("Price is trading below support, suggesting breakdown pressure.");
  }

  if (price > support && price < resistance) {
    reasons.push("Price is inside the main structure range, so confirmation is important.");
  }

  if (notes.length > 20) {
    reasons.push("Trader notes were considered as qualitative context.");
  }

  const volatility = classifyVolatility(atr, price);

  if (volatility.label === "Extreme") {
    warnings.push("Volatility is extreme; reduce risk or wait for cleaner structure.");
  }

  if (volatility.label === "High") {
    warnings.push("High ATR environment; wider stops and smaller position size are recommended.");
  }

  if (spread && price && spread / price > 0.0015) {
    warnings.push("Spread/cost is relatively high compared with price.");
  }
  ...
  if (exposurePct > 800) {
    warnings.push("Nominal exposure is very high relative to equity; use contract-specific lot sizing carefully.");
  }

  if (riskPct > 2) {
    warnings.push("Risk percentage is aggressive for a single trade.");
  }

  if (decision === "WAIT") {
    warnings.push("No clear directional edge; wait for breakout, pullback confirmation, or stronger confluence.");
  }

  const confidence = clamp(
    48 + Math.abs(edge) * 1.15 + (volatility.score - 50) * 0.28 - warnings.length * 4,
    20,
    94
  );
...
function simulateBacktest({ confidence, rr, riskPct, equity }) {
  ...
  for (let i = 0; i < trades; i++) {
    ...
    if (isWin) {
      balance += risk * rr * (0.72 + pseudo * 0.55);
      wins++;
    } else {
      balance -= risk * (0.75 + pseudo * 0.45);
      losses++;
    }
...
function buildNarrative(symbol, decision, confidence, grade, reasons, warnings, timeframe) {
  return `The model classifies ${symbol} as ${decision} on the ${timeframe} context with a ${Math.round(
    confidence
  )}% confidence score and grade ${grade}. The decision is based on trend structure, RSI position, volatility, support/resistance behavior, and risk controls. ${reasons
    .slice(0, 3)
    .join(" ")} ${
    warnings.length
      ? "Key caution: " + warnings[0]
      : "No critical warning was detected from the supplied inputs."
  }`;
}
...
app.post("/api/analyze", async (req, res) => {
  ...
  try {
    const prompt = `You are a professional trading risk analyst. Analyze this setup and return ONLY valid JSON. Keep all numeric risk levels realistic. Do not promise profit. Required JSON keys: decision, confidence, grade, reasons, warnings, aiNarrative. Input: ${JSON.stringify(
      req.body
    )} Local baseline: ${JSON.stringify({
    ...
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: "Return compact valid JSON only. This is educational analysis, not financial advice."
        },
        {
          role: "user",
          content: prompt
...
if (process.env.NODE_ENV !== "production") {
  app.listen(PORT, () => {
    console.log(`AI Trading Advisor running on http://localhost:${PORT}`);
